...
<p>The following resources have been deleted from the <?php echo $outcomeName;?> outcome:</p>
<?php	
	$postCheck = 0;
	wp_reset_postdata();
	$args = array(
	...
	if (!(strpos($extraOutcomes, $outcomeID) !== false)) {
		$postCheck = 1;
				...
				?>
				<div class="column1">
					<button class="restore-from-deleted" id="extraID<?php echo $extraPostID;?>" type="button">Restore</button>
				</div><!--column1-->
	...
	if ($postCheck == 0) {
	echo "<p><em>Sorry, there are no deleted Extras resources associated with the ".$outcomeName." outcome.</em></p>";
	}
?>
