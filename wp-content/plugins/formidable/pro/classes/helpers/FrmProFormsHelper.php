...
<?php
    ...
    public static function repeat_field_set($field_name, $args = array() ) {
        ...
        echo '<div id="frm_section_'. $args['parent_field']['id'] .'-'. $args['i'] .'" class="frm_repeat_'. ( empty($format) ? 'sec' : $format ) .' frm_repeat_'. $args['parent_field']['id'] .'">' . "\n";
    ...
    public static function repeat_buttons($args, $end = false) {
        ...
        if ( ! $end ) {
            ...
            // get end field
            $query = $wpdb->prepare('fi.form_id=%d AND type=%s AND field_order > %d', $args['parent_field']['form_id'], 'end_divider', $args['parent_field']['field_order']);
            $end = (array) FrmField::getAll($query, 'field_order', 1);

            foreach ( array('format', 'add_label' ,'remove_label') as $o ) {
    ...
    public static function repeat_button_html($args) {
        ...
        $triggers = '<div class="frm_form_field frm_'. $args['label_pos'] .'_container '. ( empty($args['field_class']) ? '' : 'frm_last_'. $args['field_class'] ) .'">';
    ...
    public static function hide_conditional_fields($frm_vars) {
        ...
        if ( ! empty($display_none) ) {
            $display_none = array_unique($display_none);
            ?>frmFrontForm.hideCondFields('<?php echo json_encode($display_none) ?>');<?php
        }

        if ( ! empty($trigger_check) ) {
            $trigger_check = array_unique($trigger_check);
            ?>frmFrontForm.checkDependent('<?php echo json_encode($trigger_check) ?>');<?php
        }
    }
