    ...
    */
    ...
    public static function formidable_shortcode_atts($atts, $all_atts){
        ...
        if ( $atts['entry_id'] == 'last' ) {
            ...
            if ( $user_ID ) {
                $frm_vars['editing_entry'] = $wpdb->get_var( $wpdb->prepare('SELECT id FROM '. $wpdb->prefix .'frm_items WHERE form_id = %d AND user_id = %d ORDER BY created_at DESC', $atts['id'], $user_ID) );
            }
...
<?php
        ...
        $opts = array(
            ...
            'include_js' => array('val' => 0, 'label' => __('Do not include the graph javascript', 'formidable')),
