function frmFrontFormJS(){
	...
	function hideOrShowField(i, f, field_id, selected, rec, repeatField){
		if ( typeof show_fields[f.HideField] == 'undefined' ) { 
			show_fields[f.HideField] = [];
		}

        var inputName = 'item_meta['+ f.FieldName +']';
        var containerID = 'frm_field_'+ f.FieldName +'_container';
        var hideContainerID = 'frm_field_'+ f.HideField +'_container';
        if ( typeof repeatField !== 'undefined' ) {
            inputName = repeatField.attr('name');
            // Accommodate for "other" options
            inputName = inputName.replace( '[other]', '' );
            var container = repeatField.closest('.frm_form_field');
            if ( container.length ) {
                containerID = container.attr('id');
                hideContainerID = containerID.replace(f.FieldName, f.HideField);
            }
        }

		if ( f.FieldName != field_id || typeof selected == 'undefined' || selected == 'und' ) {
			if ( f.Type=='radio' || f.Type=='data-radio' ) {
				selected = jQuery('input[name="'+ inputName +'"]:checked, input[type="hidden"][name="'+ inputName +'"]').val();
			} else if ( f.Type=='select' || f.Type=='time' || f.Type=='data-select' ) {
				selected = jQuery('select[name^="'+ inputName +'"], input[type="hidden"][name^="'+ inputName +'"]').val();
				var multiple = jQuery('input[type="hidden"][name^="'+ inputName +'"]');
                if ( multiple.length ) {
					selected = [];
				...
				selected = jQuery('input[name^="'+ inputName +'"], textarea[name^="'+ inputName +'"]').val();
			}
		}

		if ( typeof selected == 'undefined' ) {
			selected = jQuery('input[type=hidden][name^="'+ inputName +'"]').val();
			if ( typeof selected == 'undefined' ) {
                // check for repeating/embedded field
                if ( typeof repeatField === 'undefined' ) {
                    var repeat = jQuery('.'+ containerID +' input, '+ containerID +' select, '+ containerID +' textarea');
                    if ( repeat.length ) {
	    ...
	    if ( f.Type=='checkbox' || (f.Type=='data-checkbox' && typeof(f.LinkedField)=='undefined') ) {
            ...
            for ( var a = 0; a<checkboxes.length; a++ ) {
                if (( checkboxes[a].type == 'checkbox' && checkboxes[a].checked ) || checkboxes[a].type == 'hidden' ){
                    checkVals.push( checkboxes[a] );
                }
            }
            ...
            if ( checkVals.length ) {
                ...
                for ( var a = 0; a<checkVals.length; a++ ) {
        			match = operators(f.Condition,f.Value,jQuery(checkVals[a]).val());
                    if ( f.Condition == '!=' ) {
			...
			if ( typeof f.DataType == 'undefined' || f.DataType === '' || f.DataType === 'data' ) {
				if ( selected === '' ) {	
					show_fields[f.HideField][i] = false;
					jQuery(document.getElementById(hideContainerID)).fadeOut('slow');
			...
			var checked_vals = [];
			jQuery('input[name="'+ inputName +'[]"]:checked, input[type="hidden"][name="'+ inputName +'[]"]').each(function(){checked_vals.push(jQuery(this).val());});
			if(typeof(f.DataType) == 'undefined' || f.DataType === '' || f.DataType === 'data'){
		...
		hideFieldNow(i, f, rec, hideContainerID);
	}

	function hideFieldNow(i, f, rec, hideContainerID){
		if ( f.MatchType == 'all' ) {
			hide_later.push({
				'result':show_fields[f.HideField][i], 'show':f.Show,
                'match':f.MatchType, 'fname':f.FieldName, 'fkey':f.HideField,
                'hideContainerID':hideContainerID
			});
			return;
		}
		...
		if ( show_fields[f.HideField][i] !== false ) {
			if ( f.Show == 'show' ) {
				if ( show_fields[f.HideField][i] !== true ) {
					...
					var hideMe = document.getElementById(hideContainerID);
					if ( hideMe !== null ) {
